import type { Metadata } from "next";
import Script from "next/script";
import Image from "next/image";
import Link from "next/link";

export const metadata: Metadata = {
  title: "Certificado Salud Mental para Trabajo en Chillán | Gonzalo Pedrosa",
  description:
    "Certificado de salud mental para trabajo en Chillán. Documento profesional para postulaciones laborales, licencias y otros trámites.",
  alternates: {
    canonical: "/certificado-salud-mental-trabajo-chillan",
  },
  openGraph: {
    title: "Certificado Salud Mental para Trabajo en Chillán | Gonzalo Pedrosa",
    description: "Certificado de salud mental para trabajo en Chillán. Documento profesional para postulaciones laborales, licencias y otros trámites.",
    url: "https://www.gonzalopedrosa.cl/certificado-salud-mental-trabajo-chillan",
    type: "website",
    images: [{ url: "/yo.png", width: 1200, height: 630, alt: "Gonzalo Pedrosa - Psicólogo" }],
  },
  twitter: {
    card: "summary_large_image",
    title: "Certificado Salud Mental para Trabajo en Chillán",
    description: "Certificado de salud mental para trabajo en Chillán. Documento profesional para postulaciones laborales, licencias y otros trámites.",
  },
};


// FAQPage Schema para rich snippets en Google
const faqSchema = {
  "@context": "https://schema.org",
  "@type": "FAQPage",
  "mainEntity": [
    {
      "@type": "Question",
      "name": "¿Cuánto cuesta?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Contáctame por WhatsApp indicando para qué lo necesitas y te doy el valor exacto."
      }
    },
    {
      "@type": "Question",
      "name": "¿Qué pasa si tengo un diagnóstico?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Tener un diagnóstico no necesariamente impide obtener el certificado. Depende del diagnóstico, su estado actual y los requerimientos del trámite."
      }
    },
    {
      "@type": "Question",
      "name": "¿Es lo mismo que un informe psicológico?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "No. El certificado es más breve y general. Si necesitas un{\" \"} informe detallado {\" \"} con evaluación completa, es otro proceso."
      }
    }
  ]
};

export default function CertificadoSaludMentalTrabajoChillanPage() {
  return (
    <>
      <Script
        id="faq-schema"
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema) }}
      />
      <main style={{ maxWidth: 640, margin: "0 auto", padding: "4rem 1.5rem", fontFamily: 'system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "2rem" }}>
        <Image src="/yo.png" alt="Gonzalo Pedrosa - Psicólogo" width={64} height={64} priority style={{ borderRadius: "9999px", objectFit: "cover" }} />
        <div>
          <p style={{ fontWeight: 600, marginBottom: "0.125rem" }}>Gonzalo Pedrosa</p>
          <p style={{ fontSize: "0.875rem", color: "#666" }}>Psicólogo Clínico · +7 años de experiencia</p>
        </div>
      </div>

      <p style={{ fontSize: "0.75rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.1em", color: "#999", marginBottom: "0.75rem" }}>Chillán, Chile</p>

      <h1 style={{ fontSize: "2.5rem", fontWeight: 700, letterSpacing: "-0.03em", marginBottom: "1rem", lineHeight: 1.1 }}>
        Certificado Salud Mental para Trabajo en Chillán
      </h1>

      <p style={{ fontSize: "1.125rem", color: "#666", marginBottom: "3rem", lineHeight: 1.6 }}>
        Emito certificados de salud mental para fines laborales: postulaciones
        de trabajo, procesos de selección, licencias y otros trámites que
        requieran este documento.
      </p>

      <section style={{ marginBottom: "3rem" }}>
        <h2 style={{ fontSize: "0.75rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.1em", color: "#999", marginBottom: "1rem" }}>
          Usos comunes
        </h2>
        <ul style={{ listStyle: "none", padding: 0, display: "grid", gap: "0.5rem" }}>
          {[
            "Postulación a empleos públicos",
            "Procesos de selección en empresas",
            "Trabajos con manejo de armas",
            "Cargos de responsabilidad",
            "Trámites de residencia o visa de trabajo",
            "Reincorporación laboral post-licencia",
          ].map((item, i) => (
            <li key={i} style={{ display: "flex", alignItems: "flex-start", gap: "0.75rem", padding: "0.75rem 1rem", backgroundColor: "#fafafa", borderRadius: "8px", fontSize: "0.9375rem", color: "#444" }}>
              <span style={{ color: "#000", flexShrink: 0 }}>—</span>
              {item}
            </li>
          ))}
        </ul>
      </section>

      <section style={{ marginBottom: "3rem" }}>
        <h2 style={{ fontSize: "0.75rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.1em", color: "#999", marginBottom: "1rem" }}>
          El proceso
        </h2>
        <div style={{ display: "grid", gap: "0.75rem" }}>
          {[
            { paso: "1", titulo: "Agenda", desc: "Contacta por WhatsApp indicando para qué necesitas el certificado." },
            { paso: "2", titulo: "Evaluación", desc: "Entrevista breve (30-45 min) para evaluar tu estado actual." },
            { paso: "3", titulo: "Emisión", desc: "Si corresponde, emito el certificado el mismo día o al siguiente." },
          ].map((item, i) => (
            <div key={i} style={{ display: "flex", gap: "1rem", padding: "1rem", backgroundColor: "#fafafa", borderRadius: "8px" }}>
              <span style={{ fontSize: "1.25rem", fontWeight: 700, color: "#000", flexShrink: 0 }}>{item.paso}.</span>
              <div>
                <p style={{ fontWeight: 600, marginBottom: "0.25rem", color: "#000" }}>{item.titulo}</p>
                <p style={{ fontSize: "0.875rem", color: "#666" }}>{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
      </section>

      <section style={{ padding: "1.25rem", backgroundColor: "#f0fdf4", borderRadius: "12px", marginBottom: "2.5rem", border: "1px solid #bbf7d0" }}>
        <p style={{ fontWeight: 600, color: "#166534", marginBottom: "0.5rem", fontSize: "0.9375rem" }}>✓ Documento profesional</p>
        <p style={{ color: "#14532d", fontSize: "0.875rem", lineHeight: 1.6 }}>
          El certificado incluye firma y timbre de psicólogo titulado.
          Es válido para los fines laborales que lo requieran.
        </p>
      </section>

      <section style={{ marginBottom: "3rem" }}>
        <h2 style={{ fontSize: "0.75rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.1em", color: "#999", marginBottom: "1rem" }}>
          Preguntas frecuentes
        </h2>
        <div style={{ display: "grid", gap: "0.5rem" }}>
          <details style={{ padding: "1rem", border: "1px solid #eaeaea", borderRadius: "12px" }}>
            <summary style={{ cursor: "pointer", fontWeight: 500, color: "#000" }}>¿Cuánto cuesta?</summary>
            <p style={{ marginTop: "0.75rem", color: "#666", lineHeight: 1.6, fontSize: "0.9375rem" }}>
              Contáctame por WhatsApp indicando para qué lo necesitas y te doy
              el valor exacto.
            </p>
          </details>

          <details style={{ padding: "1rem", border: "1px solid #eaeaea", borderRadius: "12px" }}>
            <summary style={{ cursor: "pointer", fontWeight: 500, color: "#000" }}>¿Qué pasa si tengo un diagnóstico?</summary>
            <p style={{ marginTop: "0.75rem", color: "#666", lineHeight: 1.6, fontSize: "0.9375rem" }}>
              Tener un diagnóstico no necesariamente impide obtener el
              certificado. Depende del diagnóstico, su estado actual y los
              requerimientos del trámite.
            </p>
          </details>

          <details style={{ padding: "1rem", border: "1px solid #eaeaea", borderRadius: "12px" }}>
            <summary style={{ cursor: "pointer", fontWeight: 500, color: "#000" }}>¿Es lo mismo que un informe psicológico?</summary>
            <p style={{ marginTop: "0.75rem", color: "#666", lineHeight: 1.6, fontSize: "0.9375rem" }}>
              No. El certificado es más breve y general. Si necesitas un{" "}
              <Link href="/informe-psicologico-chillan" style={{ color: "#000", textDecoration: "underline" }}>
                informe detallado
              </Link>{" "}
              con evaluación completa, es otro proceso.
            </p>
          </details>
        </div>
      </section>

      <section style={{ padding: "2rem", backgroundColor: "#000", borderRadius: "16px", marginBottom: "3rem", textAlign: "center" }}>
        <h2 style={{ fontSize: "1.25rem", fontWeight: 600, marginBottom: "0.75rem", color: "#fff" }}>
          Solicita tu certificado
        </h2>
        <p style={{ color: "rgba(255,255,255,0.7)", marginBottom: "1.5rem", fontSize: "0.9375rem" }}>
          Agenda tu evaluación y obtén el documento rápidamente.
        </p>
        <a href="https://wa.me/56968257817" target="_blank" rel="noopener noreferrer" style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", padding: "0.75rem 1.5rem", backgroundColor: "#fff", color: "#000", borderRadius: "9999px", textDecoration: "none", fontWeight: 500, fontSize: "0.875rem" }}>
          Agendar →
        </a>
      </section>

      <nav style={{ paddingTop: "2rem", borderTop: "1px solid #eaeaea" }}>
        <p style={{ fontSize: "0.75rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.1em", color: "#999", marginBottom: "0.75rem" }}>Servicios relacionados</p>
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          {[
            { label: "Certificado salud mental", href: "/certificado-salud-mental-chillan" },
            { label: "Informe psicológico", href: "/informe-psicologico-chillan" },
            { label: "Estrés laboral", href: "/psicologo-estres-laboral-chillan" },
          ].map((item, i) => (
            <Link key={i} href={item.href} style={{ color: "#666", fontSize: "0.875rem", textDecoration: "none", padding: "0.375rem 0.75rem", border: "1px solid #eaeaea", borderRadius: "9999px" }}>
              {item.label}
            </Link>
          ))}
        </div>
      </nav>
    </main>
    </>
  );
}




