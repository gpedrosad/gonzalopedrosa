# Conectar Google Search Console — AgendApp

Guía de cómo está conectado GSC en `gonzalopedrosa` y cómo replicarlo para **agendapp.com.ar**.
...
```text
Contexto: En el repo gonzalopedrosa ya hay integración con Google Search Console vía OAuth (no service account). Quiero la misma conexión en el proyecto de AgendApp (propiedad GSC: sc-domain:agendapp.com.ar).
...
Para AgendApp:
- Cambiar solo GSC_SITE_URL=sc-domain:agendapp.com.ar (si GSC lista otra URL exacta, usar esa)
- Reutilizar el mismo OAuth client + el mismo Gmail si ese Gmail ya es owner de sc-domain:agendapp.com.ar (confirmado: en listados previos aparece como siteOwner)
...
Objetivo: dejar AgendApp autenticado y capaz de sacar un reporte GSC (clics, impresiones, CTR, queries, pages) igual que gonzalopedrosa.
```
...
## Paso a paso (setup desde cero o portar a AgendApp)
...
2. Confirmar la propiedad **dominio** `agendapp.com.ar` (o la URL exacta que muestre GSC).
3. Ese mismo Gmail es el que debe autorizar el flujo OAuth local.

### 3. Variables de entorno (solo GSC)

En `.env.local` del repo AgendApp:
...
GSC_SITE_URL=sc-domain:agendapp.com.ar
...
```bash
# Opción rápida si AgendApp corre en la misma máquina y mismo Gmail:
...
2. Login con el Gmail owner de `sc-domain:agendapp.com.ar`.
...
Debe listar la propiedad y métricas. Si `GSC_SITE_URL` no coincide con lo que devuelve la API, actualizar `.env.local` con la URL exacta (ej. `sc-domain:agendapp.com.ar` o `https://www.agendapp.com.ar/`).
...
- [ ] `.env.local` con `GSC_AUTH_MODE`, `GOOGLE_OAUTH_CLIENT_PATH`, `GSC_SITE_URL=sc-domain:agendapp.com.ar`
